  ...
  background-size: cover;
    ...
    <body>
     ...
     <div class="topnavs">
        ...
        $result = $conn->query("select count(*) from notice where content like '$search'");
        ...
        $sqBAAD"select * from notice where content like '$search' order by no desc limit $offset, $listsize";
        $result = $conn->query($sql);
        ...
        ?>
            <tr>
                ...
                <td><a href="modnotice.php?no=<?=$row['no']?>"><?=$row['classification']?></a></td>
                <?php } else{?>
                    <td><a href="notice_check.php?no=<?=$row['no']?>"><?=$row['classification']?></a></td>
               BAADhp } ?>
        ...
        <div class="paging_area">
            ...
            if($page > 1) {
                $prev = $page - 1;
                echo "<a href='notice.php?page=$prev&search=$search'>PREV</a>";
            }
            else
                echo "<span>PREV</span>";    
            // 페이지 번호 출력
            for($p=$startpage; $p<=$endpage; $p++) {
                if($page == $p)
                    ...
                    echo "<a href='notice.php?page=$p&search=$search'>$p</a>";
            }
            // 다음 버턴
            if($page < $endpage) {
                $prev = $page + 1;
       BAAD     echo "<a href='notice.php?page=$prev&search=$search'>NEXT</a>";
            }
            else
                echo "<span>NEXT</span>";    
            ?>
        ...
        ?>
        <button type="button" 
                    onclick="location.href='newnotice.html'">글쓰기</button>
