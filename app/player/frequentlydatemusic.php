...
<style>
  ...
  background-size: cover;
    ...
    <div class="topnavs">
        ...
        <h1>재생 목록</h1>
        <div class="topnavss">
            <div class="topnavs-center">
                <a href ="myplaylist.php?page=1">내가 추가한곡</a>
                <a href="frequentlydatemusic.php?page=1&freday=7">최근에 재생한곡</a>
                <a href="countmusic.php?page=1">많이 재생한곡</a>
               
                
            </div>
        </div>
        <div class="topnavss">
            <div class="topnavs-centers">
             
                <a href="frequentlydatemusic.php?page=1&freday=7">7일전</a>
                <a href="frequentlydatemusic.php?page=1&freday=15">15일전</a>
                <a href="frequentlydatemusic.php?page=1&freday=30">30일전</a>
                </div>
        ...
        <div class="topnav">
            <div class="search-container">
                <form action="music_chart.php?page=1" method="get">  
        ...
        $result = $conn->query("select count(*) from frequentlymusic where uid='$uid' ");
        ...
        $sql = "select * from frequentlymusic,music_list  where frequentlymusic.music_number=music_list.music_number and date >= date_add(now(), interval -$freday day) and uid='$uid' order by frequentlymusic.date  limit $offset, $listsize";
        
        $result = $conn->query($sql);
        if($result->num_rows > 0) {
            $i=1;
        ?>
        <form action=play2.php method="get" name='myForm'>
        <table id="pizza">
            <tr>
        ...
        ?>
            <tr>
                ...
                <td><a class="btn blue" href="play2.php?find=<?=$row['music_number']?>"><img src='./image/playbutton.png' width='20%'height='7%' ></a> <a class="btn blue" href="video.php?video=<?=$row['video']?>"><img src='./image/youtube.jpg' width='20%'height='7%' ></a>
                <a class="btn blue" href="javascript:popupOpen(<?=$row['music_number']?>);"><img src='./image/list.jpg' width='20%'height='4.5%' >  </a></td></td>
        ...
        <input type="number" name="frquentlydate"  value="1" hidden>
        <div class="paging_area">
            <?php
        
            if($page > 1) {
                $prev = $page - 1;
                echo "<a href='frequentlydatemusic.php?page=$prev&search=$search'>PREV</a>";
            }
            else
                echo "<span>PREV</span>";    
            ...
            for($p=$startpage; $p<=$endpage; $p++) {
                if($page == $p)
                    ...
                    echo "<a href='frequentlydatemusic.php?page=$p&search=$search'>$p</a>";
            }
            ...
            if($page < $endpage) {
                $prev = $page + 1;
                echo "<a href='frequentlydatemusic.php?page=$prev&search=$search'>NEXT</a>";
            }
            else
                echo "<span>NEXT</span>";    
            ?>
            <input type="submit" value="재생">
            <input type="button" onClick ='mysub(1)' value='삭제'> 
    </form>
        </div>
        <?php } else {
